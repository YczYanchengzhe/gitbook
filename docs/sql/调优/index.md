...
所以基于最左原则 , 如果优化索引顺序可以少建索引,那么有必要优化索引的顺序.
...
- 无索引下推 : 需要四次回表 , 这里的 age 不是没有数据,而是搜索引擎不会去查看,接口问题导致的不支持

![img](../../resources/sql/b32aa8b1f75611e0759e52f5915539ac.png)

- 索引下推,只需要一次 : 

![img](../../resources/sql/76e385f3df5a694cc4238c7b65acfe1b.png)
...
- 基于主键索引进行查询时候只需要搜索主键对应的的 B+树
- 对于普通索引,需要先搜索索引树, 取出主键 ID, 在根据主键 ID 进行搜索,这个过程称为回表.
- 在索引树上可以直接获取到要查询的全部结果,不需要回表,称为覆盖索引

### 3.2 索引的维护

- B+ 树为了维护索引有序性，在插入新值的时候需要做必要的维护
**页分裂问题**
如果要插入的数据是尾插入,那么不会有问题,但是如果是中间插入,并且要插入的数据页满了,就会涉及页分裂,大大降低插入效率.
除了性能外,页分裂操作还会影响数据页的利用率,原本放在一个页中的数据,现在放在两个页中,整体的空间利用率降低了约 50%.当然,如果相邻的两个页由于删除了数据,利用率很低后,会将数据页做合并.
**自增主键**
基于自增主键的插入 : 因为主键自增,每次插入都是尾插入,不会涉及页分裂
自增主键的选择 : 因为普通索引树的叶子节点存储的是主键 的值,所以如果主键尽可能的小,也就直接减少了内存消耗,当然,如果使用场景是 kv 存储,不会有其他索引,也就不需要考虑这一点了.
...
```sql
-- 重建索引
ALTER TABLE T DROP INDEX indexName;
ALTER TABLE T ADD INDEX (indexName);
-- 重建主键索引
ALTER TABLE T DROP PRIMARY KEY;
ALTER TABLE T ADD PRIMARY KEY (id);
```

重建索引的场景 : 索引可能因为删除,或者页分裂的原因导致数据页有空洞,重建索引的过程会新建一个新的索引,把数据按顺序插入,这样页面的利用率最高,更加节省空间.

重建索引的方式 : 上面的方式不合理,因为不论删除主键还是创建主键,都会导致整个表重建,所以连着执行两个语句的话,第一个语句就白做了,可以使用 : 
