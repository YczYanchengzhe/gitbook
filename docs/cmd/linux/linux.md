...
# -m 超时时间
# -o 结果保存到文件 后接文件名
# 官方文档地址 : https://curl.se/docs/manpage.html
curl -s -m60 -o
```
...
# 指定  ' 为分隔符,并打印分隔后的第五个和第六个参数   less a.log  | awk -F'get alert from db service id|' '{print $2}'
awk -F' ' '{print $5 $6 }'
```
...
- $0:  脚本自身的名称；　　
- $1～$n:  传入脚本的第1~n个参数；
- $@: 传入脚本的所有参数；如果使用""括起来,那么以``"$1" "$2" "$3"``方式输出所有参数
- $*：传入脚本的所有参数；如果使用""括起来,那么以``"$1 $2 $3"``方式输出所有参数
- $! :  最后运行的后台Process的PID
- $$:  脚本执行的进程id；
